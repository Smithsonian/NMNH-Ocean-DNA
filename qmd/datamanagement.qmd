...
::: {.callout-note}
We recommend initially downloading sequence data to `/scratch/public/genomics/YOUR_USER_ID`. This directory has the fastest read/write speeds and the largest storage quota. From there, you can move the sequence data to the appropriate location on Store.
:::

#### GenoHub

If your sequencing was done by GenoHub, you should receive a link with instructions to download the raw sequence data. 
...
If your sequencing was done at the NMNH LAB, you will receive a Dropbox link containing your sequence data. You can directly download the sequence data to your computer, then upload to Hydra with your [method of choice](https://confluence.si.edu/spaces/HPC/pages/163152227/Transferring+Files+to+from+Hydra).

Alternatively, you can download sequence data directly from Dropbox to Hydra using [rclone](https://rclone.org/). Advantages of `rclone`:
...
See instructions [here](https://rclone.org/dropbox/) for accessing Dropbox with rclone.

`rclone` example usage:

    - Perform a "dry run" (list files but do not copy):
```bash
rclone copy -v -n DROPBOX_REMOTE_NAME:/PATH/TO/DATA/ON/DROPBOX /PATH/TO/DESTINATION/ON/HYDRA
```
    - Copy files:
```bash
copy -v DROPBOX_REMOTE_NAME:/PATH/TO/DATA/ON/DROPBOX /PATH/TO/DESTINATION/ON/HYDRA
```
    - Copy only files that end in ".fastq.gz"
```bash
rclone copy -v --include "*.fastq.gz" DROPBOX_REMOTE_NAME:/PATH/TO/DATA/ON/DROPBOX /PATH/TO/DESTINATION/ON/HYDRA
```

### STEP 2: create the sequence data map file

The map file is a comma-separated values (CSV) file that lists all samples in the sequencing project. It is crucial for ensuring your data are interpretable.

#### GenoHub

If you sequenced with GenoHub, please use the following naming convention for your map file:

```
genohub-{GENOHUB-PROJECT-NUMBER}_{PROJECT-DESCRIPTION}_mapfile.csv
```

For example: `genohub-8459898_Vietnam_mapfile.csv`

Where "8459898" is the GenoHub project number and "Vietnam" is your description of the sequencing project. ***Do not include underscores or spaces in the project description.***


#### NMNH LAB

If you sequenced with the NMNH LAB, please use the following naming convention for your map file:

```
LAB-{LAB-RUN-NAME}_{PROJECT-DESCRIPTION}_mapfile.csv
```

For example: `LAB-240925VL008474AAG5MNMM5_IRL-2024-07_mapfile.csv`

Where "240925VL008474AAG5MNMM5" is the LAB sequencing run name (with underscores removed) and "IRL-2024-07" is your description of the sequencing project. ***Do not include underscores or spaces in the project description.***

The sequencing run name can be found in the HTML report included in the Dropbox folder from LAB. 

![LAB sequencing run report](/qmd/images/LAB_run_name.png)

#### Map file requirements

The first five columns of your map file must be:

- ***ID:*** GenoHub/LAB sample name. For example:
  - `I-0025-99AD-1N1`
...
**1. /store/nmnh_ocean_dna/public/raw_sequence_data**
</span>

Each directory within `raw_sequence_data` should contain the results of a sequencing project. 
To save disk space, please ensure that all sequence files are compressed (e.g. `sequence1.fastq.gz`).

#### GenoHub

For GenoHub sequencing projects, please use the following naming convention for your directories:

```
genohub-{GENOHUB-PROJECT-NUMBER}_{PROJECT-DESCRIPTION}
```
...
For LAB sequencing projects, please use the following naming convention for your directories:

```
LAB-{LAB-RUN-NAME}_{PROJECT-DESCRIPTION}
```

For example: `LAB-240925VL008474AAG5MNMM5_IRL-2024-07`
...
```bash
cd /store/nmnh_ocean_dna/public/raw_sequence_data
...
<span style="font-size: 20px;">
**2. /store/nmnh_ocean_dna/public/raw_sequence_metadata**
</span>
...
Prior to copying new map files to the P drive, you should make a backup of the existing directory. Open the `store` directory in the terminal and run the following commands. For `YEAR-MONTH-DAY`, please use the format `2025-09-26`.

```bash
cp -r raw_sequence_metadata raw_sequence_metadata_backups/raw_sequence_metadata_YEAR-MONTH-DAY
cd raw_sequence_metadata_backups
tar -zcvf raw_sequence_metadata_YEAR-MONTH-DAY.tar.gz raw_sequence_metadata_YEAR-MONTH-DAY
rm -r raw_sequence_metadata_YEAR-MONTH-DAY
```
...
```bash
cd /Volumes/nmnh-ocean-dna/Hydra_backup/store/public/raw_sequence_metadata
rclone copy -v -n SI-Hydra:/store/nmnh_ocean_dna/public/raw_sequence_metadata/ . 
```
...
```bash
rclone copy -v SI-Hydra:/store/nmnh_ocean_dna/public/raw_sequence_metadata . 
```
...
rclone copy -v -n SI-Hydra:/store/nmnh_ocean_dna/public/raw_sequence_data . 
```

And again, if the dry run looks good, proceed with transfer.

```bash
rclone copy -v SI-Hydra:/store/nmnh_ocean_dna/public/raw_sequence_data . 
```
