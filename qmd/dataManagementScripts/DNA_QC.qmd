...
[Source code](https://github.com/dmacguigan/SI-Ocean-DNA/blob/main/scripts/data_managements/DNA_QC/run_DNA_QC.sh)
...
wget https://github.com/dmacguigan/SI-Ocean-DNA/blob/main/scripts/data_managements/DNA_QC/DNA_QC.nf
# download the Nextflow config file
wget https://github.com/dmacguigan/SI-Ocean-DNA/blob/main/scripts/data_managements/DNA_QC/Hydra.nf.config
```

## Running the script

Decide where you want to store the QC reports and navigate there on the command line. Then download the [`run_DNA_QC.sh`](https://github.com/dmacguigan/SI-Ocean-DNA/blob/main/scripts/data_managements/DNA_QC/run_DNA_QC.sh) script. For example:

```bash
mkdir ~/my_project_QC # create a new directory in your home directory
cd ~/my_project_QC 
wget https://github.com/dmacguigan/SI-Ocean-DNA/blob/main/scripts/data_managements/DNA_QC/run_DNA_QC.sh
```
...
If you have many samples, you may wish to consider running `run_DNA_QC.sh` as a batch job. Note that you will need to use the special workflow manager queue "lTWFM.sq" in order to run Nextflow from a batch job.
